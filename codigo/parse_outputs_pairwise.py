...
def convertir_a_ranking(d, orden):
    valores = [d.get(s) for s in orden]
    valores_validos = [v for v in valores if v is not None]
    unicos_ordenados = sorted(set(valores_validos), reverse=True)
    valor_a_ranking = {v: i+1 for i, v in enumerate(unicos_ordenados)}
    return {s: valor_a_ranking.get(d.get(s)) for s in orden}
...
def process_json_to_table(data, src_lang):
    ...
    ranking_por_instancia = {}

    # create all players
    players = {
        "enes-eu": trueskill.Rating(),
        "latxa": trueskill.Rating(),
        "gt": trueskill.Rating(),
    }
    ...
    if src_lang == "es":
        players["es-eu"] = trueskill.Rating()
        modelos["es-eu"] = 0
    elif src_lang == "en":
        players["en-eu"] = trueskill.Rating()
        modelos["en-eu"] = 0
    elif src_lang == "en-es":
        players["en-eu"] = trueskill.Rating()
        players["es-eu"] = trueskill.Rating()
    ...
    for inst in data:
        ...
        #print(comb)
        if ranking_por_instancia.get(inst["code"]) is None:
            ranking_por_instancia[inst["code"]] = modelos.copy()
            ranking_por_instancia[inst["code"]]["language"] = inst["language"]
            ranking_por_instancia[inst["code"]]["corpus"] = inst["corpus"]
            ranking_por_instancia[inst["code"]]["ita"] = inst["ita"]
            ranking_por_instancia[inst["code"]]["id"] = inst["id"]
        ...
        if winner=="a":
            ...
            winner_model = comb[0]
            loser_model = comb[1]
            ranking_por_instancia[inst["code"]][comb[0]] += 1

        elif winner=="b":
            suma_winners[comb[1]] += 1
            winner_model = comb[1]
            loser_model = comb[0]
            ranking_por_instancia[inst["code"]][comb[1]] += 1
        else:
            print(f"Error al parsear winner en code {inst['code']}:\n{winner}")
    ...
    return cont_ranking, trueskill_ranking, ranking_por_instancia
    

def main(input_file, output_file, csv_out):
    ...
    for lang in ["es", "en"]:
        ...
        cont_ranking, trueskill_ranking, ranking_por_instancia = process_json_to_table(data_lang, lang)
        
        orden_sistemas = ['enes-eu', 'gt', 'en-eu', 'latxa', 'es-eu']

        output_csv = "../rankings_per_instance/"+csv_out.replace(".csv", f"_{lang}.csv")

        with open(output_csv, "w", newline='', encoding='utf-8') as csvfile:
            fieldnames = ['Language', 'code','ita','corpus'] + orden_sistemas
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
            writer.writeheader()

            for id_, datos in ranking_por_instancia.items():
                # Extraer Language y corpus
                language = datos.get('language', '')
                corpus = datos.get('corpus', '')
                ita = datos.get('ita', '')
                code = datos.get('id', '')
                
                # Extraer solo los sistemas
                sistemas = {k: v for k, v in datos.items() if k in orden_sistemas}
                
                # Convertir a ranking
                ranking = convertir_a_ranking(sistemas, orden_sistemas)
                
                # Preparar fila para el CSV
                fila = {'Language': language, 'corpus': corpus, 'ita': ita, 'code': code}
                fila.update(ranking)
    ...
    for corpus in corpus_names:
        ...
        cont_ranking_corpus, trueskill_ranking_corpus, _ = process_json_to_table(data_corpus, corpus.split("-")[1])    
...
if __name__ == "__main__":
    ...
    main(args.input_file, output_path, input_filename.replace(".json", ".csv"))
