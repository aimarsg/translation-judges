...
def main(data_path, prompt_path, model, batch_size, output_path):
    ...
    with open(data_path, 'r') as dataset:
        ...
        for instancia in data:
            ...
            for key in ["enes-eu", "gt", "latxa"]:
                if key in instancia:
                    translations[key] = instancia[key]
            
            if "en-eu" in instancia:
                translations["en-eu"] = instancia["en-eu"]
            if "es-eu" in instancia:
                translations["es-eu"] = instancia["es-eu"]
    ...
    for response in responses:
        ...
        # (adrian) ANADIRIA: (para luego sacar rankings a diferentes niveles: language, corpus...)
        json_output["id"] = instance_metadata[i]["id"]
        json_output["corpus"] = instance_metadata[i]["corpus"]
        json_output["code"] = instance_metadata[i]["code"]
        json_output["language"] = instance_metadata[i]["language"]
        json_output["ita"] = instance_metadata[i]["ita"]
...
if __name__ == "__main__":
    ...
    parser.add_argument("model", help="llama/gemma/mistral/aloe/latxa", choices=["llama", "gemma", "mistral", "aloe", "latxa"])
